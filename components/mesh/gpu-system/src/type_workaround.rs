...
// /// Currently, the naga only support struct in bind array of storage buffer, so we have to wrap
// the /// array into the struct, so we have to impl some trait by hand
// #[repr(transparent)]
...
// impl<T: ShaderSizedValueNodeType> ShaderMaybeUnsizedValueNodeType for
// BindlessStorageWorkaround<T> {   const MAYBE_UNSIZED_TYPE: MaybeUnsizedValueType = todo!();
// }
