...
export function intoThree() {
    ...
    const arraySize = 30;
    console.log("item count", arraySize * arraySize * arraySize);
    const grid = 2;
...
export function intoWasmScene() {
    ...
    const position_data = new AttributeBufferF32WASM(new Float32Array(data), 3);
    const position_buffer = new VertexBufferWASM(viewer, position_data);
    ...
    for (let i = 0; i < arraySize; i++) {
        ...
        root.add_child(node);
        for (let j = 0; j < arraySize; j++) {
            const node2 = new SceneNodeWASM(viewer);
            // node2.setPosition(0, j * grid, 0);
            node.add_child(node2);
            for (let k = 0; k < arraySize; k++) {
                const node3 = new SceneNodeWASM(viewer);
