...
pub struct ShaderStructMetaInfo {
  ...
  /// Only exists for host shareable struct, see [ShaderStructHostLayout].
  pub host_layout: Option<ShaderStructHostLayout>,
}

/// The memory layout of a host shareable struct.
///
/// The offsets and size come directly from the rust type(generated by the layout macro), the
/// shader backend must use them as is, so the shader side layout is always identical to the host
/// data. The backend is responsible to check they are valid for the WGSL layout rules.
#[derive(Clone, PartialEq, Eq, Debug, Hash)]
pub struct ShaderStructHostLayout {
  pub target: ShaderHostLayoutTarget,
  /// the byte offset of each field, in the same order of the fields
  pub field_offsets: Vec<usize>,
  pub size: usize,
}

/// Which address space the host shareable struct is designed for.
#[derive(Clone, Copy, PartialEq, Eq, Debug, Hash)]
pub enum ShaderHostLayoutTarget {
  /// for uniform buffer
  Std140,
  /// for storage buffer
  Std430,
...
impl ShaderStructMetaInfo {
  ...
  #[must_use]
  pub fn with_host_layout(mut self, layout: ShaderStructHostLayout) -> Self {
    assert_eq!(layout.field_offsets.len(), self.fields.len());
    self.host_layout = Some(layout);
    self
  }
