...
// pub type ShaderGraphNodeRawHandle = usize;

pub trait GraphicsMultiShaderAPI {
  type VertexCtx: VertexShaderAPI;
  type FragmentCtx: FragmentShaderAPI;
  fn build_vertex(&mut self) -> Self::VertexCtx;
  fn build_fragment(&mut self) -> Self::FragmentCtx;
}

pub trait ShaderSemanticRegistry {
  fn register<T>(&mut self, value: ShaderValueHandle);
  fn query<T>(&mut self) -> Option<ShaderValueHandle>;
}

pub trait ComputeShaderAPI: ShaderControlFlowAPI + ShaderSemanticRegistry {
  fn set_workgroup_size(&mut self);
  fn workgroup_id(&mut self) -> ShaderValueHandle;
}

pub trait VertexShaderAPI: ShaderControlFlowAPI + ShaderSemanticRegistry {
  fn vertex_id(&mut self) -> ShaderValueHandle;
}
pub trait FragmentShaderAPI: ShaderControlFlowAPI + ShaderSemanticRegistry {
  fn frag_id(&mut self) -> ShaderValueHandle;
}

pub type ShaderValueHandle = usize;

pub trait ShaderExprAPI {
  fn add(&mut self, a: ShaderValueHandle, b: ShaderValueHandle) -> ShaderValueHandle;
}

pub trait ShaderControlFlowAPI: ShaderExprAPI {
  type ShaderConditionalCtx: ShaderControlFlowAPI;
  type ShaderWhileCtx: ShaderControlFlowAPI;
  fn if_by(&mut self) -> Self::ShaderConditionalCtx;
  fn while_by(&mut self) -> Self::ShaderWhileCtx;
}
...
//     }
//   }
//   pub fn visit_single<R>(&self, mut visitor: impl FnMut(&ShaderValueSingleType) -> R) ->
// Option<R> {     match self {
...
// // `let v: <rendiation_algebra::Vec4<f32> as ShaderFieldTypeMapper>::ShaderType =
// Vec4::default();` //
