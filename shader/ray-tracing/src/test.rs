...
async fn test_wavefront_compute() {
  ...
  use rendiation_webgpu::GPUBuffer;
  ...
  let buffer = GPUBuffer::create(
    &gpu.device,
    BufferInit::WithInit(&[12, 34, 56, 78]),
    BufferUsages::MAP_READ | BufferUsages::COPY_DST | BufferUsages::COPY_SRC,
  );

  let buffer = GPUBufferResource::create_with_raw(
    buffer,
    GPUBufferDescriptor {
  ...
  let buffer = {
    ...
    let buffer = encoder.read_buffer_bytes(&gpu.device, &buffer);
    println!("read buffer start");
    gpu.submit_encoder(encoder);
    println!("read buffer await");
    let r = buffer.await.unwrap();
    println!("read buffer finish");
    r
  };
  println!("{buffer:?}");
  ...
  let buffer = {
    ...
    println!("read texture start");
    gpu.submit_encoder(encoder);
    println!("read texture await");
    let r = buffer.await.unwrap();
    println!("read texture finish");
    r
  };
  ...
  use std::ops::Deref;
  prefix.extend_from_slice(buffer.deref());
