...
impl TaskGroupExecutorResource {
  ...
  pub fn create_with_size(
    ...
    Self {
      active_task_idx_back_buffer: allocator.allocate((size * 4) as u64, device, "active_task_idx"),
  ...
  pub fn init(&self, cx: &mut DeviceParallelComputeCtx) {
    ...
    cx.record_pass(|pass, device| {
      let hasher = shader_hasher_from_marker_ty!(PrepareEmptyIndices).with_hash(&self.payload_ty);

      let workgroup_size = 256;
      let pipeline = device.get_or_cache_create_compute_pipeline_by(hasher, |mut builder| {
        builder.config_work_group_size(workgroup_size);

        let empty_pool = builder.bind_by(&self.empty_index_pool.storage);
        let empty_pool_size = builder.bind_by(&self.empty_index_pool.current_size);
      ...
      let mut builder = BindingBuilder::default()
        .with_bind(&self.empty_index_pool.storage)
        .with_bind(&self.empty_index_pool.current_size);
