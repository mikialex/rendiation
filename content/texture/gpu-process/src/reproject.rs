...
impl GPUReprojectInfo {
  ...
  pub fn update(&self, ctx: &mut FrameCtx, current_mvp_no_translation: Mat4<f32>) {
    let current_mvp_no_translation_inv = current_mvp_no_translation.inverse_or_identity();
    self.reproject.mutate(|d| {
      ...
      d.current_camera_view_projection = current_mvp_no_translation;
    });
