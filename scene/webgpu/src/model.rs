...
//     material_delta:
// ctx.get_or_create_reactive_material_render_component_delta_source(&s.material),     mesh_id:
// s.mesh.guid(),     mesh_delta:
// ctx.get_or_create_reactive_mesh_render_component_delta_source(&s.mesh),     group: s.group,
//   };
//   drop(s);
...
//   fn create_scene_reactive_gpu(&self, ctx: &GPUModelResourceCtx) -> Option<ReactiveModelGPUType>
// {     match self {
//       Self::Standard(model) => ReactiveModelGPUType::Standard(build_standard_model_gpu(model,
// ctx)),       Self::Foreign(m) => get_dyn_trait_downcaster_static!(WebGPUSceneModel)
...
//   impl AsRef<RenderComponentCell<ReactiveSceneModelGPU>> + Stream<Item =
...
//   let model_delta =
// ctx.get_or_create_reactive_model_render_component_delta_source(&source.model);
...
//           let model_delta =
// ctx.get_or_create_reactive_model_render_component_delta_source(&model);           state.inner.
// model_id = model_id;           state.inner.model_delta = model_delta;
//           RenderComponentDeltaFlag::ContentRef
//         }
