...
#[shader_fn]
pub fn offset_ray_hit(position: Node<Vec3<f32>>, offset_dir: Node<Vec3<f32>>) -> Node<Vec3<f32>> {
  let int_scale = val(256.0);
  let float_scale = val(1.0 / 65536.0);
  let origin = val(1.0 / 32.0);

  let of_i = offset_dir * int_scale.splat::<Vec3<_>>();
  let of_i = of_i.into_i32();
  ...
  let r_x = position
    ...
    .abs()
    .less_than(origin)
    .select(position.x() + float_scale * offset_dir.x(), p_i_x);
  let r_y = position
    .y()
    .abs()
    .less_than(origin)
    .select(position.y() + float_scale * offset_dir.y(), p_i_y);
  let r_z = position
    .z()
    .abs()
    .less_than(origin)
    .select(position.z() + float_scale * offset_dir.z(), p_i_z);
