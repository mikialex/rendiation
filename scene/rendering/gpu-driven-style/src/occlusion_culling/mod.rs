...
struct DeviceOcclusionCulling<T> {
  adaptor: T,
  last_frame_visibility: HashMap<SceneCameraId, StorageBuffer<[bool]>>,
}

pub trait DeviceOcclusionCullingContent: SceneRenderingAdaptor {
...
impl<T: DeviceOcclusionCullingContent> SceneRenderingAdaptor for DeviceOcclusionCulling<T> {
