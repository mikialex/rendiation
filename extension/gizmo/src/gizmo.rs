...
//             if let DeltaOf::<AxisActiveState>::$a(_) | DeltaOf::<AxisActiveState>::$b(_) =
// v.delta {               let s = v.data;
//               let both = ItemState {
...
// ) -> Option<Mat4<f32>> {
//   #[rustfmt::skip]
//   // new_hit_world = M(parent) * M(local_translate) * M(new_local_rotate) * M(local_scale) *
// start_hit_local_position =>   //  M-1(local_translate) * M-1(parent) * new_hit_world =
// M(new_local_rotate) * M(local_scale) * start_hit_local_position   // should we support world
// space point align like above? but the question is, we have to also modify scale, because   //
