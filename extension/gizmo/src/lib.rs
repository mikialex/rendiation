    ...
    use_inject_cx::<Option<DragStartState>>(cx, |cx| {
      use_group(cx, |cx, root| {
        ...
        cx.on_event(|cx, _, dcx| {
          ...
          if let Some(window_state) = cx.platform_event.current_window_state() {
            if start_states.is_some() && window_state.state_delta.mouse_position_change {
            ...
            if window_state.state_delta.is_left_mouse_releasing() {
