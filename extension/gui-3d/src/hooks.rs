...
#[track_caller]
pub fn group(cx: &mut UI3dCx, children: impl FnOnce(&mut UI3dCx, EntityHandle<SceneNodeEntity>)) {
  cx.scoped(|cx| {
...
pub fn use_node_entity<'a>(cx: &'a mut UI3dCx) -> &'a mut EntityHandle<SceneNodeEntity> {
  cx.use_state_init(|cx| cx.writer.node_writer.new_entity())
}
...
pub fn use_scene_model_entity<'a>(
  ...
  cx.use_state_init(|cx| create(&mut cx.writer))
}
