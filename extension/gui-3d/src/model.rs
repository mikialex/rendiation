...
pub struct UIModel {
  ...
  on_mouse_click: Option<Box<dyn FnMut(&mut View3dCtx, Vec3<f32>)>>,
  on_mouse_hovering: Option<Box<dyn FnMut(&mut View3dCtx, Vec3<f32>)>>,
  on_mouse_down: Option<Box<dyn FnMut(&mut View3dCtx, Vec3<f32>)>>,
...
impl UIModel {
  ...
  pub fn with_on_mouse_click(mut self, f: impl FnMut(&mut View3dCtx, Vec3<f32>) + 'static) -> Self {
    self.on_mouse_click = Some(Box::new(f));
    self
  }
  pub fn with_on_mouse_hovering(
    mut self,
    f: impl FnMut(&mut View3dCtx, Vec3<f32>) + 'static,
  ) -> Self {
    self.on_mouse_hovering = Some(Box::new(f));
    self
  }
  pub fn with_on_mouse_down(mut self, f: impl FnMut(&mut View3dCtx, Vec3<f32>) + 'static) -> Self {
