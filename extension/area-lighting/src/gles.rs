...
pub type PerSceneAreaLightUniform = FastHashMap<
  RawEntityHandle,
  UniformBufferDataView<UniformArrayWithLengthInfo<LTCAreaLightUniform>>,
>;
pub type AreaLightUniforms = (
  PerSceneLightUniformArray<LTCAreaLightUniform>,
  Arc<RwLock<PerSceneAreaLightUniform>>,
);

pub fn use_area_per_scene_uniform_array_buffers(
  cx: &mut QueryGPUHookCx,
) -> Option<AreaLightUniforms> {
  ...
  let uniform_array_caches = cx.use_shared_hash_map("area light uniform_array_caches");
...
pub struct SceneAreaLightingProvider {
  ...
  pub uniform: LockReadGuardHolder<PerSceneAreaLightUniform>,
}
...
impl LightSystemSceneProvider for SceneAreaLightingProvider {
  fn get_scene_lighting(
    ...
    let lights = self.uniform.get(scene.raw_handle_ref())?.clone();
