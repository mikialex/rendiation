...
impl Bvh {
  ...
  /// - [`cast_ray`](Self::cast_ray) - Ray3<f32> casting queries
  ///
  /// # Performance
  ///
  ...
  ///
  /// ```
  /// use dynamic_bvh::{Bvh, BvhBuildStrategy, TraversalAction};
  ...
  ///
  /// let query_region = Box3::new(Vec3::new(-1.0, -1.0, -1.0), Vec3::new(7.0, 2.0, 2.0));
  ///
  /// let mut count = 0;
  /// bvh.traverse(|node| {
  ...
  /// let bvh = Bvh::from_leaves(BvhBuildStrategy::default(), &aabbs);
  ///
  /// let target_point = Vec3::new(5.5, 0.5, 0.5);
  /// let mut found_leaf = None;
  ///
  /// bvh.traverse(|node| {
  ///   if !node.aabb().contains(&target_point) {
  ///     return TraversalAction::Prune;
  ///   }
  ///
  ...
  /// - [`cast_ray`](Self::cast_ray) - Ray3<f32> casting with best-first traversal
  ...
  pub fn find_best<L: BvhLeafCost>(
    &self,
    max_cost: f32,
    aabb_cost: impl Fn(&BvhNode, f32) -> f32,
    leaf_cost: impl Fn(u32, f32) -> Option<L>,
  ) -> Option<(u32, L)> {
    ...
    loop {
      ...
      if left_score < best_cost && left_score != f32::MAX {
        if left.is_leaf() {
      ...
      if right_score < best_cost && right_score != f32::MAX {
        if right.is_leaf() {
