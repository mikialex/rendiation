...
impl Bvh {
    ...
    if let Some(leaf) = self.leaf_node_indices.get(leaf_index as usize) {
      ...
      if change_detection_margin > 0.0 {
        if !node.contains_aabb(&aabb) {
          node.min = aabb.min - Vec3::splat(change_detection_margin);
          node.max = aabb.max + Vec3::splat(change_detection_margin);
  ...
  fn insert_new_unchecked(&mut self, aabb: Box3<f32>, leaf_index: u32) {
    ...
    loop {
      ...
      let left_cost = left_merged_vol * (left_count + 1) as f32 + right_vol * right_count as f32;
      let right_cost = right_merged_vol * (right_count + 1) as f32 + left_vol * left_count as f32;
      ...
      if left_cost < right_cost || (left_cost == right_cost && left_count < right_count) {
        ...
        if left.is_leaf() {
          ...
          self.leaf_node_indices[left.children as usize] = BvhNodeIndex::left(new_leaf_id as u32);
          self.leaf_node_indices[leaf_index as usize] = BvhNodeIndex::right(new_leaf_id as u32);

          left.children = new_leaf_id as u32;
          left.data.add_leaf_count(1);
          left.min = left.min.min(aabb.min);
          left.max = left.max.max(aabb.max);
          break;
        } else {
          let left = &mut self.nodes[curr_id as usize].left;
        ...
        if right.is_leaf() {
          ...
          self.leaf_node_indices[leaf_index as usize] = BvhNodeIndex::left(new_leaf_id as u32);
          self.leaf_node_indices[right.children as usize] = BvhNodeIndex::right(new_leaf_id as u32);

          right.children = new_leaf_id as u32;
          right.data.add_leaf_count(1);
          right.min = right.min.min(aabb.min);
          right.max = right.max.max(aabb.max);
          break;
        } else {
          let right = &mut self.nodes[curr_id as usize].right;
  ...
  fn maybe_apply_rotation(&mut self, node_id: u32) {
    ...
    let curr_score =
      left.volume() * left.leaf_count() as f32 + right.volume() * right.leaf_count() as f32;
    ...
    macro_rules! eval_costs {
      ($left: ident, $right: ident) => {
        if !$left.is_leaf() {
          ...
          let new_score1 = left_child.volume() * left_child.leaf_count() as f32
            + right_child.merged_volume($right)
              * (right_child.leaf_count() + $right.leaf_count()) as f32;

          // New SAH score after transforming [{left_child, right_child}, right]
          // into [right_child, {left_child, right}].
          let new_score2 = right_child.volume() * right_child.leaf_count() as f32
            + left_child.merged_volume($right)
              * (left_child.leaf_count() + $right.leaf_count()) as f32;
    ...
    if rotation_score0 < 0.0 || rotation_score1 < 0.0 {
      ...
      if rotation_score0 < rotation_score1 {
        ...
        if left_child_moves_up0 {
          ...
          self.nodes[node_id as usize].right = self.nodes[children_id as usize].merged(children_id);

          if left_child_is_leaf {
            self.leaf_node_indices[left_child_leaf_data as usize] = BvhNodeIndex::left(node_id);
          } else {
            self.parents[left_child_leaf_data as usize] = BvhNodeIndex::left(node_id);
          }
          if right_is_leaf {
            self.leaf_node_indices[right_leaf_data as usize] = BvhNodeIndex::left(children_id);
          } else {
          ...
          if right_child_is_leaf {
            self.leaf_node_indices[right_child_leaf_data as usize] = BvhNodeIndex::left(node_id);
          } else {
            self.parents[right_child_leaf_data as usize] = BvhNodeIndex::left(node_id);
          }
          if right_is_leaf {
            self.leaf_node_indices[right_leaf_data as usize] = BvhNodeIndex::right(children_id);
          } else {
        ...
        if left_child_moves_up1 {
          ...
          self.nodes[node_id as usize].left = self.nodes[children_id as usize].merged(children_id);
          if left_child_is_leaf {
            self.leaf_node_indices[left_child_leaf_data as usize] = BvhNodeIndex::right(node_id);
          } else {
            self.parents[left_child_leaf_data as usize] = BvhNodeIndex::right(node_id);
          }
          if left_is_leaf {
            self.leaf_node_indices[left_leaf_data as usize] = BvhNodeIndex::left(children_id);
          } else {
