...
impl Bvh {
  ...
  pub(crate) fn rebuild_range_binned(&mut self, target_node_id: u32, leaves: &mut [BvhNode]) {
    ...
    let k1 = NUM_BINS as f32 * (1.0 - BIN_EPSILON) / (bins_range[1] - bins_range[0]);
    let k0 = bins_range[0];
    for leaf in &*leaves {
      let bin_id = (k1 * (leaf.center()[bins_axis] - k0)) as usize;
      let bin = &mut bins[bin_id];
      bin.aabb.expand_by_other(leaf.aabb());
      bin.leaf_count += 1;
    }
    ...
    for i in 1..NUM_BINS - 1 {
      right_acc
        .aabb
        .expand_by_other(right_merges[NUM_BINS - 1 - i].aabb);
    ...
    for i in 0..NUM_BINS - 1 {
      ...
      let cost = left_merge.aabb.volume() * left_merge.leaf_count as f32
        + right.aabb.volume() * right.leaf_count as f32;
      if cost < best_cost {
        best_cost = cost;
        best_plane = i;
      ...
      left_merge.aabb.expand_by_other(bins[i + 1].aabb);
    ...
    if left_leaves.len() == 1 {
      ...
      if target.left.is_leaf() {
        self.leaf_node_indices[target.left.children as usize] = BvhNodeIndex::left(target_node_id);
      } else {
    ...
    if right_leaves.len() == 1 {
      ...
      self.nodes[target_node_id as usize].right = self.nodes[right_id as usize].merged(right_id);
    }
  }
}
