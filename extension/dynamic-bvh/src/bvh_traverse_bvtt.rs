  ...
  fn self_intersect_node<const CHANGE_DETECTION: bool>(
    ...
    if left_right_intersect {
      match (left_is_leaf, right_is_leaf) {
        ...
        (true, false) => {
          self.traverse_single_subtree::<CHANGE_DETECTION>(workspace, &node.left, right_child, f)
        }
        (false, true) => {
          self.traverse_single_subtree::<CHANGE_DETECTION>(workspace, &node.right, left_child, f)
        }
        (false, false) => {
          self.traverse_two_branches::<CHANGE_DETECTION>(workspace, left_child, right_child, f)
        }
      }
    }
  }
