// NOTE: this is copied from https://github.com/contain-rs/vec-map/blob/master/src/lib.rs
// (currently under passive maintenance) with the following modifications:
// - Removal of the allocator type parameter.
// - Ported to std.
...
///
/// ```
/// use dynamic_bvh::vec_map::VecMap;
///
...
impl<V> VecMap<V> {
  ...
  pub fn drain(&mut self) -> Drain<'_, V> {
    ...
    let filter: fn((usize, Option<V>)) -> Option<(usize, V)> = filter;
  ...
  pub fn insert(&mut self, key: usize, value: V) -> Option<V> {
    ...
    let was = self.v[key].replace(value);
    if was.is_none() {
      self.n += 1;
    }
    was
...
impl<V> PartialOrd<VecMap<V>> for VecMap<V>
where
  V: PartialOrd,
{
  #[inline]
...
pub struct Drain<'a, V> {
  iter:
    FilterMap<Enumerate<vec::Drain<'a, Option<V>>>, fn((usize, Option<V>)) -> Option<(usize, V)>>,
}
...
mod test {
  ...
  #[test]
  fn test_move_iter() {
    let mut m: VecMap<Box<isize>> = VecMap::new();
