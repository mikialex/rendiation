...
pub fn collect_face_surface_data(table: &Table) -> Result<Vec<FaceSurfaceData>, StepReadError> {
  ...
  for (&brep_id, brep_holder) in &table.manifold_solid_brep {
    ...
    if let Some(shell_id) = entity_id_from_ph(&brep_holder.outer) {
      crate::step::step_dbg!("step: brep #{brep_id} → shell #{shell_id}");
      collect_from_shell_id(shell_id, table, &mut faces);
    } else {
...
fn collect_from_shell_id(shell_id: u64, table: &Table, faces: &mut Vec<FaceSurfaceData>) {
  ...
  for oface_ph in &shell.shell_element {
    if let Some(oface_id) = entity_id_from_ph(oface_ph) {
      ...
      if table.oriented_face.contains_key(&oface_id) {
        collect_from_oriented_face_id(oface_id, table, faces);
      } else if table.face_surface.contains_key(&oface_id) {
        collect_from_face_surface_id(oface_id, table, faces);
      } else {
        crate::step::step_dbg!(
...
fn collect_from_oriented_face_id(oface_id: u64, table: &Table, faces: &mut Vec<FaceSurfaceData>) {
  ...
  faces.push(FaceSurfaceData { surface, edges });
}
...
fn collect_from_face_surface_id(face_id: u64, table: &Table, faces: &mut Vec<FaceSurfaceData>) {
  let face = match table.face_surface.get(&face_id) {
    Some(f) => f,
    None => {
