...
pub fn convert_bspline_surface_to_bezier_patches(
  ...
  // Compute parameter range for each patch from knot spans
  build_patch_param_ranges(patches, &u_knots, &v_knots, u_degree, v_degree)
}
...
pub fn convert_bezier_surface_to_patch(
  ...
  let surface = RationalBezierSurface::new(flat_cp, u_degree, v_degree);
  PatchParamRange {
    surface,
    u_range: (0.0, 1.0),
    v_range: (0.0, 1.0),
...
/// extent.
pub fn convert_plane_to_bezier_patch(position: &Axis2Placement3d) -> PatchParamRange {
  ...
  // 2×2 grid of control points forming a unit square in the plane
  let extent = 1.0; // arbitrary; pcurve trim curves define the actual boundary
  let corners = [
...
pub fn convert_cylinder_to_bezier_patches(
  ...
  for i in 0..4 {
    ...
    // Arc control points at bottom and top
    ...
    // 3×2 control point grid (u_degree=2, v_degree=1)
    // Row-major: [v0_row, v1_row]
    let cp = vec![
...
pub fn convert_cone_to_bezier_patches(
  ...
  for i in 0..4 {
    ...
    // Radius at each v level: r(v) = radius - v * tan(semi_angle)
    // Actually, in STEP, the cone's radius parameter is at the location point.
    // The radius at distance d along axis = radius - d * tan(semi_angle)
    // (if semi_angle is positive, the cone narrows)
...
pub fn convert_sphere_to_bezier_patches(
  ...
  let center = cartesian_point_to_vec3(&position.location);
  let polar = direction_to_vec3(&position.axis).normalize(); // north pole direction
  let x_dir = axis2_x_dir(position);
...
pub fn convert_extrusion_surface_to_bezier_patches(
...
) -> Result<Vec<PatchParamRange>, StepReadError> {
  ...
  for seg in curve_segments {
    ...
    // Interleave bottom and top rows
...
pub fn convert_any_surface_to_bezier_patches(
  ...
  match surface {
    ...
    SurfaceAny::BezierSurface(b) => {
      ...
      let u_degree = b.u_degree as usize;
      let v_degree = b.v_degree as usize;
      Ok(vec![convert_bezier_surface_to_patch(
        u_degree,
        v_degree,
        control_points,
      )])
    ...
    SurfaceAny::Plane(p) => Ok(vec![convert_plane_to_bezier_patch(&p.position)]),
    SurfaceAny::CylindricalSurface(c) => Ok(convert_cylinder_to_bezier_patches(
      &c.position,
      c.radius,
      None, // v_range from trim curves at assembly time
    )),
...
mod tests {
  ...
  fn cylinder_patch_evaluates_on_cylinder() {
    ...
    let patches = convert_cylinder_to_bezier_patches(&position, 2.0, Some((0.0, 5.0)));
  ...
  fn sphere_patch_evaluates_on_sphere() {
    ...
    let patches = convert_sphere_to_bezier_patches(&position, radius);
