...
// impl<T1, T2, K, V1, V2, F, O> ReactiveCollection<K, O> for ReactiveKVUnion<T1, T2, K, F, O, V1,
// V2> where   K: Copy + std::hash::Hash + Eq + Send + Sync + 'static,
//   F: Fn((Option<V1>, Option<V2>)) -> Option<O> + Send + Sync + Copy + 'static,
...
//   fn poll_changes(&mut self, cx: &mut Context<'_>) -> CPoll<CollectionTransaction<K, O>> {
//     let waker = cx.waker().clone();
//     let (t1, t2) = rayon::join(
//       || {
//         let mut cx = Context::from_waker(&waker);
...
//               .map(|v| (k, v))
//           })
//           .collect::<CollectionTransaction<K, O>>()
//       }
