...
impl<T: GPULinearStorage> ResizableLinearStorage for ResizableGPUBuffer<T> {
  fn resize(&mut self, new_size: u32) -> bool {
    self.resize_with_relocations(new_size, None)
  }
}

impl<T: GPULinearStorage> RelocationResizableLinearStorage for ResizableGPUBuffer<T> {
  fn resize_with_relocations(
    ...
    // relocations may be required even if the size is unchanged, in that case we still
    // go through the resize path so they are applied on a fresh copy safely
    if self.gpu.max_size() != new_size || relocations.is_some() {
