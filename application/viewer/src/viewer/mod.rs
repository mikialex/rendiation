...
impl Viewer3dSceneDeriveSource {
  fn poll_update(&self) -> Viewer3dSceneDerive {
    ...
    let (_, world_mat) = self.world_mat.describe(cx).resolve();
    let (_, node_net_visible) = self.node_net_visible.describe(cx).resolve();
    let (_, camera_transforms) = self.camera_transforms.describe(cx).resolve();
    ...
    let (_, sm_world_bounding) = self.sm_world_bounding.describe(cx).resolve();
