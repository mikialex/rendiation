...
impl Viewer3dSceneDeriveSource {
  fn poll_update(&self) -> Viewer3dSceneDerive {
    ...
    let (_, world_mat) = self.world_mat.poll_changes(cx).resolve();
    let (_, node_net_visible) = self.node_net_visible.poll_changes(cx).resolve();
    let (_, camera_transforms) = self.camera_transforms.poll_changes(cx).resolve();
    ...
    let (_, sm_world_bounding) = self.sm_world_bounding.poll_changes(cx).resolve();
