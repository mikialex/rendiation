...
pub fn viewer_rendering_logic(cx: &mut Viewer3dRenderingCx) {
  use_viewer_texture_system(cx, |cx| {
    use_light_system(cx, |cx, lighting| {
...
fn use_scene_renderer(
  ...
  let renderer: &dyn SceneRenderer<ContentKey = SceneContentKey> = match render_type {
    Gles => {
      use_gles_scene_renderer(cx, reversed_depth, attributes_custom_key, texture_system);
      todo!()
    }
    Indirect => use_indirect_renderer(cx, reversed_depth, texture_system),
  };
