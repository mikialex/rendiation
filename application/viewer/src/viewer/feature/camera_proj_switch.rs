...
pub fn use_camera_proj_switch(cx: &mut ViewerCx) {
  ...
  if let ViewerCxStage::SceneContentUpdate { writer } = &mut cx.stage
    && let Some(CameraSwitchProjAction { camera, proj }) = switch_proj_action.take()
  {
    match proj {
  ...
  if cx.is_resolve_stage()
    && let Some(SuitableOrthComputeRequest { camera, look_at }) =
  ...
  if let ViewerCxStage::EventHandling { .. } = cx.stage
    && let Some(RequestSwitchCameraProjType(camera)) = raw_request.take()
  {
