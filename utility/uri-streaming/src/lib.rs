...
pub trait AbstractResourceStreaming: Send + Sync {
  type UriLike;
  type Data;
  type Key;
  ...
  ///
  /// if load result is none, it means the item is paged in in but failed to load
  fn poll_loading(
    &mut self,
    cx: &mut Context,
    loader: &mut LoaderFunction<Self::UriLike, Self::Data>,
...
impl<K: CKey, V: CValue, U> AbstractResourceStreaming
  for Box<dyn AbstractResourceStreaming<Data = V, Key = K, UriLike = U>>
{
  type UriLike = U;
  type Data = V;
  type Key = K;
