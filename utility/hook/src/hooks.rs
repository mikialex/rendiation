...
pub unsafe trait HooksCxLike: Sized {
  ...
  #[track_caller]
  fn skip_if_not<R>(&mut self, should_execute: bool, f: impl FnOnce(&mut Self) -> R) -> Option<R> {
  ...
  fn use_sharable_plain_state<T: 'static>(&mut self, f: impl FnOnce() -> T) -> Arc<RwLock<T>> {
    let (_, r) = self.use_plain_state(|| Arc::new(RwLock::new(f())));
    r.clone()
  }
}
...
fn create_key_from_hash_impl<K: std::hash::Hash>(key: &K, key_id: u32) -> SubFunctionKeyType {
