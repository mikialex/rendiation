---
name: rendiation-algebra
description: >
  Reference for the rendiation math library (math/algebra). Covers Vec2/Vec3/Vec4, Mat2/Mat3/Mat4,
  Quat, the Scalar trait (Float + FloatConst + ScalarConstEval), generic float-constant
  construction via scalar_transmute/eval, InnerProductSpace (length2/dot), VectorSpace,
  and SpaceEntity. Use when writing generic math code over T: Scalar, working with
  vectors/matrices, or needing to create float literals in generic contexts.
metadata:
  version: "1.0"
  updated: "2026-05-18"
---

The `math/algebra` crate is the foundational math library. Import everything with:

```rust
use rendiation_algebra::*;
```

## Scalar trait

`Scalar` is a trait alias that bundles all numeric capabilities needed for generic math code:

```rust
pub trait Scalar = Float
  + AsPrimitive<i64>
  + FloatConst
  + ScalarConstEval
  + Copy
  + std::fmt::Debug
  + AddAssign<Self> + SubAssign<Self> + DivAssign<Self> + MulAssign<Self>
  + Send + Sync + Default
  + 'static;
```

This means `T: Scalar` is only implemented for `f32` and `f64`. It provides:
- `T::zero()`, `T::one()` — from `num_traits::Zero`/`One`
- `T::abs()`, `T::sqrt()` — from `Float`
- Arithmetic ops (`+`, `-`, `*`, `/`, `+=`, `-=`, `*=`, `/=`) — from `Float` + `*Assign` traits
- Comparison (`<`, `>`, `==`) — from `PartialOrd` (via `Float`)
- `T::from(small_int)` — from `NumCast` (e.g. `T::from(3).expect(...)`)

## Generic float constants via ScalarConstEval

`ScalarConstEval` provides compile-time float-to-generic conversion via `f32` bit patterns:

```rust
pub trait ScalarConstEval: Sized {
  fn eval<const N: u32>() -> Self;   // f32::from_bits(N).into()
  fn half() -> Self;                 // 0.5
  fn two() -> Self;                  // 2.0
  fn three() -> Self;                // 3.0
  fn pi_by_c180() -> Self;           // PI / 180.0
  fn c180_by_pi() -> Self;           // 180.0 / PI
  fn by_usize_div(a: usize, b: usize) -> Self;
}

impl<T: From<f32>> ScalarConstEval for T { ... }
```

The helper `scalar_transmute` converts a float literal to a `u32` bit pattern for use in const generics:

```rust
pub const fn scalar_transmute(value: f32) -> u32 { value.to_bits() }
```

**Usage pattern** — to write `1e-7` in generic code:

```rust
let threshold: T = T::eval::<{ scalar_transmute(1e-7) }>();
```

Common constants: `T::half()` (0.5), `T::two()` (2.0), `T::one()`, `T::zero()`. Also available: `T::pi_by_c180()` (PI / 180.0, degrees to radians), `T::c180_by_pi()` (180.0 / PI, radians to degrees), and `T::by_usize_div(a, b)` (ratio of two `usize` values as float).

## Vector types

All generated by `impl_vector!` macro. Available: `Vec2<T>`, `Vec3<T>`, `Vec4<T>`.

**Constructors:**
```rust
let v = Vec3::new(x, y, z);
let v = Vec4::new(x, y, z, w);
```

**Key traits (all require `T: Scalar` for full functionality):**

| Trait | Provides | File |
|-------|----------|------|
| `InnerProductSpace<T>` | `length2()`, `dot()`, `length()` | [vec/dimension.rs](math/algebra/src/vec/dimension.rs) |
| `VectorSpace<T>` | `Add`, `Sub`, `Mul<T>`, `Div<T>` | [vec/dimension.rs](math/algebra/src/vec/dimension.rs) |
| `Functor` | `f_map(f)` | [lib.rs](math/algebra/src/lib.rs) |

Vector arithmetic works element-wise: `a + b`, `a - b`, `a * scalar`, `a / scalar`, `scalar * a`.

## Matrix types

`Mat2<T>`, `Mat3<T>`, `Mat4<T>` — square matrices. `Mat4::identity()` for identity.

## Quaternion

`Quat<T>` — quaternion for rotations.

## f32/f64 conversion

Inherent methods on every vector/matrix/quat type, generated by the `f32_f64_convert!` macro (not by the `Functor` trait):

```rust
v.into_f64()  // Vec3<f32> → Vec3<f64>
v.into_f32()  // Vec3<f64> → Vec3<f32>
```

## SpaceEntity

For types that can be transformed by matrices:

```rust
pub trait SpaceEntity<T: Scalar, const D: usize> {
  type Matrix: SquareMatrixDimension<D>;
  fn apply_matrix(&mut self, mat: Self::Matrix) -> &mut Self;
}
```

## Key files

| File | Content |
|------|---------|
| [math/algebra/src/lib.rs](math/algebra/src/lib.rs) | `Scalar` trait alias, `ScalarConstEval`, `scalar_transmute`, `f32_f64_convert!` |
| [math/algebra/src/vec/mod.rs](math/algebra/src/vec/mod.rs) | `impl_vector!` macro, `vec2`/`vec3`/`vec4` constructors |
| [math/algebra/src/vec/dimension.rs](math/algebra/src/vec/dimension.rs) | `InnerProductSpace`, `VectorSpace`, `Vector` traits |
| [math/algebra/src/vec/vec3.rs](math/algebra/src/vec/vec3.rs) | `Vec3` struct, `dot_impl` |
| [math/algebra/src/vec/vec4.rs](math/algebra/src/vec/vec4.rs) | `Vec4` struct |
| [math/algebra/src/mat/](math/algebra/src/mat/) | `Mat2`, `Mat3`, `Mat4` |
| [math/algebra/src/quat.rs](math/algebra/src/quat.rs) | `Quat` |
