...
impl<'a, V: PositionedPoint3, T: PrimitiveTopology<V> + WGPUPrimitiveTopology> GeometryProvider
...
pub struct GPUGeometry<V: PositionedPoint3 = Vertex, T: PrimitiveTopology<V> = TriangleList> {
...
impl<V: PositionedPoint3, T: PrimitiveTopology<V>> From<IndexedGeometry<V, T>>
  for GPUGeometry<V, T>
{
...
impl<V: PositionedPoint3, T: PrimitiveTopology<V>> From<(Vec<V>, Vec<u16>)> for GPUGeometry<V, T> {
  fn from(item: (Vec<V>, Vec<u16>)) -> Self {
    IndexedGeometry::new(item.0, item.1).into()
  }
}

impl<V: PositionedPoint3, T: PrimitiveTopology<V>> GPUGeometry<V, T> {
